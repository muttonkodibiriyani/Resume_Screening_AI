...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-bg-elevated p-6 shadow-lg transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
        ...
        left: 'inset-y-0 left-0 h-full w-3/4 border-r sm:max-w-sm data-[state=open]:animate-slide-in-left data-[state=closed]:animate-slide-out-right',
        right:
  ...
  ({ side = 'right', className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <DialogPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-md p-1 text-fg-muted hover:bg-bg-muted focus:outline-none focus:ring-2 focus:ring-ring">
          <X className="h-4 w-4" />
