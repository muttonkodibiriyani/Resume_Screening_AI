...
export const GET = apiHandler(async (req: NextRequest, { params }: { params: Record<string, string> }) => {
  ...
  if (format === 'pdf') {
    ...
    doc.setFillColor(107, 31, 46); doc.rect(0, 0, 297, 22, 'F');
    doc.setTextColor(255); doc.setFontSize(16); doc.text('Alshaya AI Recruit - Candidate Ranking Report', 14, 14);
    doc.setTextColor(0); doc.setFontSize(10);
    doc.text(`Role: ${benchmark.roleTitle}`, 14, 30);
    doc.text(`Skill Family: ${benchmark.skillFamily} | Seniority: ${benchmark.seniority} | Min Exp: ${benchmark.minExperience}+ years`, 14, 36);
    doc.text(`Benchmark v${benchmark.version} | Source: ${benchmark.benchmarkSource} | Engine: ${benchmark.generationMode} | Generated: ${new Date().toLocaleString()}`, 14, 42);
    ...
    autoTable(doc, {
      ...
      body: rows.map((r) => [r.rank, r.candidate, r.score, r.band, r.engine, r.matched, r.missing, r.redFlags, r.risk, r.decision]),
    ...
    if (top3.length > 0) {
      doc.addPage();
      doc.setFillColor(107, 31, 46); doc.rect(0, 0, 297, 22, 'F');
      doc.setTextColor(255); doc.setFontSize(16); doc.text('Top 3 Recommended Candidates', 14, 14);
      doc.setTextColor(0);
      let y = 32;
      for (const r of top3) {
        ...
        doc.text(`#${r.rank} ${r.candidate}  -  ${r.score}/100 (${r.band})`, 14, y); y += 6;
        ...
        doc.text('Strengths:', 14, y); y += 4;
        strengths.forEach((s) => { doc.text(`- ${s.slice(0, 130)}`, 18, y); y += 4; });
        doc.text('Gaps:', 14, y); y += 4;
        gaps.forEach((s) => { doc.text(`- ${s.slice(0, 130)}`, 18, y); y += 4; });
        if (flags.length) {
          doc.text('Red Flags:', 14, y); y += 4;
          flags.forEach((s) => { doc.text(`- ${s.slice(0, 130)}`, 18, y); y += 4; });
        }
        y += 4;
